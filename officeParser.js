    ...
    .then((files) => {
      ...
      files.sort((a, b) => {
        const matchedANumber = parseInt(
          a.path.match(slideNumberRegex)?.at(1),
          10
        );
        const matchedBNumber = parseInt(
          b.path.match(slideNumberRegex)?.at(1),
          10
        );
        ...
        return (
          aNumber - bNumber ||
          Number(a.path.includes("notes")) - Number(b.path.includes("notes"))
        );
      });
        ...
        responseText.push(
          Array.from(sheetsXmlCNodesList)
            ...
            .map((cNode) => {
              ...
              handleError(
                `Invalid c node found in sheet xml content: ${cNode}`,
                callback,
...
function parseOffice(srcFile, callback, config = {}) {
  ...
  const filePreparedPromise = new Promise((res, rej) => {
    ...
    if (Buffer.isBuffer(file))
      ...
      if (!fs.existsSync(file)) throw ERRORMSG.fileDoesNotExist(file);
  ...
  return new Promise((res, rej) => {
    ...
    const processZipfile = (zipfile) => {
      ...
      function processEntry(entry) {
        ...
        } else zipfile.readEntry(); // Skip entries that don't match the filter
      }
...
if (
  ...
  (process.argv[1].split("/").pop() == "officeParser.js" ||
    process.argv[1].split("/").pop().toLowerCase() == "officeparser")
) {
